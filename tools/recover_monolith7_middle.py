"""Recover exact ANFs for Monolith7 output words 3–5.
...
Shared normalized polynomials are stored only once.
"""
...
def recover_model(root: Path = REPOSITORY_ROOT) -> Model:
    ...
    for word in (3, 4, 5):
        for bit in range(32):
            ...
            if key not in function_ids:
                ...
                functions.append({"inputs": len(source_ids), "terms": sorted(normalized)})
    ...
    return {"version": 1, "output_words": [3, 4, 5], "functions": functions, "bits": bits}
