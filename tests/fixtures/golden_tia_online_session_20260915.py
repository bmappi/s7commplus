...
MITM session between TIA Portal and the PLC (TLS terminated by a pinned
proxy, TOFU certificate trust). They back the answers in ANSWERS.md.
...
# -- NOTIFICATIONS -----------------------------------------------------------
#
# Two content bearing event notifications (ANSWERS.md section 5). Header after
# the subscription id: 04 00 00 00 00 00, then 00, seq VLQ, 00, an 8 byte
# block, then records starting with the 0x92 element id.
#
...
#
# TIA diagnostic subscription create (function 0x04CA). The two qualifier
# counters (ANSWERS.md section 8): the VLQ after the 00 04 00 00 00 00 marker
# is the create's WRITE slot (0x12 = 18 here), the VLQ inside the a1 7f ff c0
# prefix is the client subscription counter (0x56), mirrored in the object
# name Subscription_2147467350 = Subscription_(0x7FFFC000 + 0x56).
sub_create_req = (
...
# Its response. retval VLQ 00, then 0x0d (NOT 00, see ANSWERS.md section 4),
# then the 4 byte fill.
symbolic_write_resp = (
    b"\x72\x02\x00\x10\x32\x00\x00\x05\x7c\x00\x00\x00\x07\x34\x00\x0d\x00\x00"
    b"\x00\x00"
)
